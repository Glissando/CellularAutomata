...
//			if (LOG_LEVEL >= MB2_LogLevel.debug) MB2_Log.Log(MB2_LogLevel.debug,"Assigning atlas " + texturePath + " to result material " + target + " for property " + texPropName,LOG_LEVEL);
		...
		public int GetMaximumAtlasDimension(){
			...
			if (!Application.isPlaying){		
				...
				if (EditorUserBuildSettings.activeBuildTarget == BuildTarget.iPhone) atlasMaxDimension = 2048;
			...
			for (int i = 0; i < objsToMesh.Count; i++){
				...
				if (pt == UnityEditor.PrefabType.None ||
					...
					if (objToCombineType == MB_ObjsToCombineTypes.prefabOnly){
						Debug.LogWarning("The list of objects to combine contains scene objects. You probably want prefabs. Translation, Rotation and Scale will be baked into the generated mesh." + objsToMesh[i] + " is a scene object");	
						return false;
					}
